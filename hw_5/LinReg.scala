...
class LinearRegression(learningRate: Double, epsilon: Double, verbose_n: Int = 5000) {
  ...
  private def _computeGradients(xInput: DenseMatrix[Double], yTrue: DenseVector[Double], yPredict: DenseVector[Double]) = {
...
def shuffle(dataTrainX: DenseMatrix[Double], dataTrainY: DenseVector[Double],
    ...
    var cvDataTrainX: DenseMatrix[Double] = DenseMatrix.zeros[Double](cvRowSize, dataTrainX.cols)
    var cvDataTrainY: DenseVector[Double] = DenseVector.zeros[Double](cvRowSize)
    var cvDataTestX: DenseMatrix[Double] = DenseMatrix.zeros[Double](rows - cvRowSize, dataTrainX.cols)
    var cvDataTestY: DenseVector[Double] = DenseVector.zeros[Double](rows - cvRowSize)
    var index = Random.shuffle(for (i <- 0 until rows) yield i)
    for(i <- 0 until rows) {
      if (i < cvRowSize) {
  ...
  def crossvalItreration(model: LinearRegression, dataTrainX: DenseMatrix[Double], dataTrainY: DenseVector [Double],
  ...
  def crossValidation(model: LinearRegression, dataTrainX: DenseMatrix[Double],
    ...
    var cvResult: DenseVector[Double] = DenseVector.zeros[Double](cvSize)
    for(i <- 0 until cvSize) {
      println(s"Cross-validation iteration - ${i + 1}")
      var (cvDataTrainX, cvDataTrainY, cvDataTestX, cvDataTestY) = shuffle(dataTrainX, dataTrainY, rows, cvRowSize)
      cvResult(i) = crossvalItreration(model, cvDataTrainX, cvDataTrainY, cvDataTestX, cvDataTestY)
    }
    cvResult
  }
